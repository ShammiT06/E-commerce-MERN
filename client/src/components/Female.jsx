...
function Female() {
  ...
  const [newarr, setarr] = useState({ img: "", description: "", price: "", discount: "", size: "" });
  ...
  const handleChange = (e) => {
    setarr({ ...newarr, [e.target.name]: e.target.value });
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      setarr({ ...newarr, img: URL.createObjectURL(file) });
    }
    ...
    setarr((prev) => ({ ...prev, img: responseurl.url }));
    console.log(newarr)
  ...
  const handleSubmit = () => {
    ...
    if (!newarr.img || !newarr.description || !newarr.price || !newarr.discount || !newarr.size) {
      alert("Error Please Fill the Necessary Details")
      return
    }

    setarr([...collect, newarr])
    ...
    setarr({ img: "", description: "", price: "", discount: "", size: "" })
    console.log(newarr)
  ...
  return (<div>
    <div className="bg-slate-300 p-5 fixed top-0 left-0 w-full">
      <div className="flex items-center justify-between">
        ...
        <svg onClick={()=>{setopen(true)}} style={{cursor:"pointer"}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-10 md:hidden">
    ...
    <div className="mt-[100px]">
      <h1 className="font-bold text-3xl text-center mt-6">Laddies' Hub</h1>
    ...
    </div>
    {
      ...
      <div className="flex flex-col w-[70%] items-center justify-center m-auto ml-[15%]  mt-[20px] p-5">
        ...
        {newarr.img && <img src={newarr.img} alt="Preview" width="200" />}
        <input type="text" name="description" value={newarr.description} onChange={handleChange} placeholder="Description" className="border-2 mt-4 p-2 w-[70%] border-black" />
        <input type="number" name="price" value={newarr.price} onChange={handleChange} placeholder="Offer Price" className="border-2 mt-4 p-2 w-[70%] border-black" />
        <input type="number" name="discount" value={newarr.discount} onChange={handleChange} placeholder="Actual Price" className="border-2 mt-4 p-2 w-[70%] border-black" />
        <input type="text" name="size" value={newarr.size} onChange={handleChange} placeholder="Size Available" className="border-2 mt-4 p-2 w-[70%] border-black" />
