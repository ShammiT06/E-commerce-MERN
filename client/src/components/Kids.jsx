...
function Kids() {
    ...
    const [newar, setar] = useState({ img: "", description: "", price: "", discount: "", size: "" });
    const [admin,setadmin]=useState(false)
    const [open,setopen]=useState(false)


    const handleChange = (e) => {
        setar({ ...newar, [e.target.name]: e.target.value });
    };
    useEffect(()=>{
...
function Logout()
{
  ...
  navigate("/")
    ...
    const handleImageUpload = async (e) => {
        ...
        if (file) {
            setar({ ...newar, img: URL.createObjectURL(file) });
        }
        ...
        setar((prev) => ({ ...prev, img: responseurl.url }));
        console.log(newar)

    };

    const handleSubmit = () => {
        if (!newar.img || !newar.description || !newar.price || !newar.discount || !newar.size) {
        ...
        setar({ img: "", description: "", price: "", discount: "", size: "" })
        console.log(newar)
        ...
        (<div>
            <div className="bg-slate-300 p-5 fixed top-0 left-0 w-full">
                <div className="flex items-center justify-between">
                    ...
                    <svg onClick={()=>{setopen(true)}}  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style={{cursor:"pointer"}} className="size-10 md:hidden">
            ...
            <div className="mt-[100px]">
                <h1 className="font-bold text-3xl text-center mt-6">KiddoKart</h1>
            ...
            </div>
            {
                ...
                <div className="flex flex-col w-[70%] items-center justify-center m-auto ml-[15%]  mt-[20px] p-5">
                    ...
                    {newar.img && <img src={newar.img} alt="Preview" width="200" />}
                    <input type="text" name="description" value={newar.description} onChange={handleChange} placeholder="Description" className="border-2 mt-4 p-2 w-[70%] border-black" />
                    <input type="number" name="price" value={newar.price} onChange={handleChange} placeholder="Offer Price" className="border-2 mt-4 p-2 w-[70%] border-black" />
                    <input type="number" name="discount" value={newar.discount} onChange={handleChange} placeholder="Actual Price" className="border-2 mt-4 p-2 w-[70%] border-black" />
                    <input type="text" name="size" value={newar.size} onChange={handleChange} placeholder="Size Available" className="border-2 mt-4 p-2 w-[70%] border-black" />
